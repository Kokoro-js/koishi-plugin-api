import { Context, Dict, Logger, Time, Schema, Service, remove } from "koishi";
import {
  createHMAC,
  createSHA256,
  createSHA1,
...
export class API extends Service {
  ...
  constructor(
    ...
    const newRouter = new Router({ prefix: config.path });
  ...
  async start() {
    ...
    if (this.config.botsAPI?.enabled) {
      ...
      this.baseRouter.use(config.path, config.router.routes());
    }

    for (const layer of this.baseRouter.stack) {
      layer.name = this.layerName;
    }

    this.ctx.router.use(this.baseRouter.routes());
  }

  async stop() {
    this.ctx.router.stack = this.ctx.router.stack.filter(
  ...
  dataDecryptMiddleware = (ctx: any, next: any) => {
    const headers = ctx.request.headers;
